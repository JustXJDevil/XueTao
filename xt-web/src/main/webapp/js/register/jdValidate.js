...
function checkPin(option) {
    ...
    if (!namestate || nameold != pin) {
        if (nameold != pin) {
            ...
            $.ajax({
            	url : "http://sso.xt.com/user/check/"+escape(pin)+"/1?r=" + Math.random(),
            	dataType : "jsonp",
            	success : function(data) {
                    ...
                    if(data.status==200){
                    	...
                    	//后台运行有问题
                    	 validateSettings.error.run(option, "系统维护中请稍后~~实际上服务器错了不知道什么时候修改骗你的");
                         namestate = false;
                    }
                }
            });
...
function sendMobileCode() {
    ...
    // 检测手机号码是否存在
    $.ajax({
    	url : "http://sso.xt.com/user/check/"+mobile+"/2?r=" + Math.random(),
    	dataType : "jsonp",
